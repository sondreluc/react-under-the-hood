...
When the button is clicked, `moreAnimals` is called, which is in charge of calculating the new state and updating the state via `setState`. Once `setState` is called, React now knows the state has changes and figures out which parts of the UI need to change.
...
While this seems like a novel approach but it's actually not unique -- it is just new to web development. React took a look at another area of software development, the gaming industry, and brought those ideas to the web.
